...
const getLoc = () => {
  if(localStorage.getItem('bookmarked')) {
    return JSON.parse(localStorage.getItem('bookmarked')).filter(item => item.isBookmarked === true)
  }
  return  data.filter((item) => item.category === 'Movie').filter(item => item.isBookmarked === true)

}
const fakeTvDb = getLoc()
const BookmarkedMovies = () => {
  const {recommended, setRecommended, searchValue, bookMarkedM} = useGlobalContext();
  const [newBookMarked, setNewBookMarked] = useState(getLoc())
  ...
  const filterRecommended = useCallback(() => {
    const newVal = fakeTvDb.filter((item) => {
      if(searchValue === '') {
        return item
      }
      else {
  ...
  return (
    <div className='mt-3'>
      ...
      {isSearching ? <h3 className='text-xl md:text-3xl text-PureWhite mb-4'>{`found ${newBookMarked.length} results for '${searchValue}'`}</h3> : <h3 className='text-xl md:text-3xl text-PureWhite mb-4'>Bookmarked Movies</h3>}
